...
async function run() {
  ...
  lineChart('#equity', [{ name: displayName, type: 'strategy-nav', unit: 'nav', values: rows.map(row => ({ date: row.date, value: row.nav })) }], { capabilities: { logScale: false, table: true, range: true } }); drawdownChart('#dd', rows); if (current) window.dispatchEvent(new CustomEvent('prstk:experiment-updated', { detail: current }));
}
